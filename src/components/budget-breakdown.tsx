    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-gray-100 bg-white overflow-hidden">
        {estimate.lines.map((line, i) => (
          <div
            key={i}
            className={`flex items-start gap-3 px-4 py-3 ${i < estimate.lines.length - 1 ? "border-b border-gray-50" : ""}`}
          >
            <span className="text-lg shrink-0 mt-0.5">{line.icon}</span>
            <div className="flex-1 min-w-0">
              <div className="flex items-baseline justify-between gap-2">
                <span className="text-sm font-medium text-gray-800">{line.category}</span>
                <span className="text-sm font-mono text-gray-700 shrink-0">{fmt(line.amountPerPerson)}<span className="text-xs text-gray-400 font-sans"> /p</span></span>
              </div>
              <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">{line.notes}</p>
            </div>
          </div>
        ))}
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-gray-50 border-t border-gray-100">
          <div>
            ...
            {travelers > 1 && (
              <span className="text-xs text-gray-400 ml-2">{travelers} travelers</span>
            )}
          </div>
          <div className="text-right">
            <div className="text-sm font-semibold font-mono text-gray-900">{fmt(estimate.totalPerPerson)}<span className="text-xs text-gray-400 font-sans"> /person</span></div>
            {travelers > 1 && (
              <div className="text-xs text-gray-500 font-mono">{fmt(estimate.totalAll)} total</div>
            )}
          </div>
        </div>
      </div>
